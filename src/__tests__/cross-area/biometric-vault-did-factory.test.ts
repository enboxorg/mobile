...
jest.mock(
  ...
  () => {
    ...
    // `addPredefinedKeys`. A subclass that stores predefined keys only
    // in its own `_predefinedKeys` Map leaves the base class's
    // `_keyStore` empty, so
    ...
    // Deterministic Ed25519 signer spy used to assert signature shape
    // and invocation without depending on random signer output.
    const Ed25519 = {
...
jest.mock(
  ...
  () => {
    ...
    // Simulate DidDht.create's real signing step so key-manager
    // integration bugs surface. The real DidDht.create publishes a DID document to the
    ...
    const mockCreate = jest.fn(
      async ({
        ...
        // This call fails if `LocalKeyManager.sign()` cannot find the
        // key in its own empty `_keyStore`. The override must
        // short-circuit to `_predefinedKeys.get(keyUri)` +
        // `Ed25519.sign(...)` so the call succeeds.
...
describe('BiometricVault + default didFactory integration (DeterministicKeyGenerator.sign)', () => {
  it('initialize() with the REAL default didFactory produces a did:dht BearerDid and does NOT throw "Key not found"', async () => {
    ...
    // A regression fails with `Error: Key not found: urn:jwk:...`
    // originating from the inherited `LocalKeyManager.sign()`.
    // Using a try/catch + explicit
