...
export const VAULT_ERROR_CODES = [
  ...
  // surfaced when a concurrent
  // generateAndStoreSecret/getSecret/deleteSecret is already in flight on the
...
async function defaultDidFactory({
  ...
  try {
    ...
    // zero the per-identity HD child keys' private key
 ...
 * non-hex character.
 *
 * the previous implementation used a bare
...
/**
 * explicit zero-out of an HDKey instance's sensitive
...
async function deriveContentEncryptionKey(rootHdKey: any): Promise<Uint8Array> {
  ...
  // ensure the vault HDKey buffers are zeroed even if
...
/**
 * Produce standard compact JWE (`dir` / `A256GCM`) with the protected
 * header bound as AAD. WebCrypto joins ciphertext and tag internally,
 * so this helper splits them into the JWE ciphertext and tag segments.
 */
async function aesGcmEncrypt(cek: Uint8Array, plaintext: Uint8Array): Promise<string> {
  ...
  // Bind the protected header's base64url form as AES-GCM AAD, as
  // required by compact JWE.
...
async function aesGcmDecrypt(cek: Uint8Array, jwe: string): Promise<Uint8Array> {
  ...
  // an empty tag segment used to be silently accepted
  // because WebCrypto would happily decrypt zero-tagged ciphertext
  // produced by the previous encoder; reject it explicitly so a
...
export function mapNativeErrorToVaultError(err: unknown): VaultError | null {
  ...
  switch (code) {
    ...
    case 'VAULT_ERROR_OPERATION_IN_PROGRESS':
      // native module rejected because a concurrent
      // generateAndStoreSecret / getSecret / deleteSecret is already running on
  ...
  implements IdentityVault<{ InitializeResult: string }>
{
  ...
  // Serialize initialize/unlock across native state transitions while
  // preserving same-method promise memoization.
  ...
  async initialize(params: {
    ...
    // install pending slot SYNCHRONOUSLY (so a
  ...
  private async _doInitialize(params: {
    ...
    // Refuse to overwrite a pre-existing native secret. A failed
    // hasSecret probe is indeterminate and must fail closed.
    ...
    // Derive the canonical 32-byte wallet entropy before touching the
    // native layer so provisioning never needs an immediate read-back.
    ...
    // Pass the derived entropy and provisioning prompt to native.
    // iOS uses the prompt for an explicit LAContext confirmation before
    // SecItemAdd, since Keychain insertion itself does not prompt.
    try {
      ...
      if (mapped?.code === 'VAULT_ERROR_KEY_INVALIDATED') {
        ...
        try {
          ...
          // Log the persist failure with full context so on-call can
          // correlate stuck-on-BiometricUnlock
    ...
    //    real root cause.
    // hold the local rootHdKey in `let` so the
    ...
    try {
      ...
      //    Do NOT store rootHdKey on `this`. The field
      ...
      if (this._secureStorage) {
        // The native secret is already provisioned here. Persist failures
        // are observable, but orphan-secret recovery can still route from
        // the native hasSecret probe on the next launch.
        try {
          ...
          console.warn(
            "[biometric-vault] failed to persist post-initialize SecureStorage flags (INITIALIZED + biometricState=ready); the vault is fully provisioned and the orphan-secret recovery in session-store.hydrate() will route the next launch correctly via the hasSecret=true probe, but the routing-fast-path flags are stale:",
            persistErr,
          );
        }
      }
      ...
      // scrub the local rootHdKey's chain code +
  ...
  async unlock(
    ...
    // symmetric counterpart of the initialize() guard
  ...
  private async _doUnlock(retainSecretForBackup = false): Promise<void> {
    ...
    if (!hasExisting) {
      // iOS auto-deletes biometry-current-set Keychain
      ...
      // proves the user already had a working vault. When that signal
      // exists AND the native item is gone, route as
      // `KEY_INVALIDATED` so the UI surfaces RecoveryRestore.
      //
      // The same path also covers Android's fixed
      ...
      if (wasInitialized) {
        ...
        // material before throwing.
        this._clearInMemoryState();
        this._biometricState = 'invalidated';
        try {
          await this._persistBiometricState('invalidated');
        } catch (persistErr) {
          // Log and continue. The KEY_INVALIDATED
          ...
          console.warn(
            '[biometric-vault] failed to persist biometricState=invalidated after detecting a missing native secret on an initialized vault; next launch routing may take an extra unlock retry to flip to RecoveryRestore:',
            persistErr,
          );
        }
        throw new VaultError(
    ...
    try {
      ...
      const mapped = mapNativeErrorToVaultError(err);
      if (mapped?.code === 'VAULT_ERROR_KEY_INVALIDATED') {
        // Clear any resident derived material before reporting key
        // invalidation; cached DID/CEK no longer map to a recoverable vault.
        this._clearInMemoryState();
        this._biometricState = 'invalidated';
        try {
          await this._persistBiometricState('invalidated');
        } catch (persistErr) {
          // The KEY_INVALIDATED throw still propagates; a later unlock can
          // retry this persist.
          console.warn(
      ...
      // If the item disappears between hasSecret() and getSecret(), use
      // SecureStorage signals to distinguish key invalidation from a fresh install.
      if (mapped?.code === 'VAULT_ERROR_NOT_INITIALIZED') {
        ...
        if (wasInitialized) {
          ...
          try {
            ...
            // Log and continue. The KEY_INVALIDATED throw below remains
            // the user-facing error for this iOS biometry-current-set race.
            console.warn(
              '[biometric-vault] failed to persist biometricState=invalidated after NOT_FOUND→KEY_INVALIDATED disambiguation (iOS biometry-current-set path); next launch routing may take an extra unlock retry to flip to RecoveryRestore:',
              persistErr,
            );
          }
          throw new VaultError(
    ...
    // Rebuild derived state atomically. Any derivation failure must zero
    // local sensitive bytes and clear stale resident vault state.
    ...
    try {
      ...
      // Scrub the local rootHdKey buffers on every
  ...
  /**
   * Wipe native vault state, SecureStorage flags, and in-memory material.
   * Missing native aliases are idempotent; other native failures propagate
   * so agent-store can keep retry sentinels armed.
   */
  async reset(): Promise<void> {
    // Capture the first durable cleanup failure, but always run in-memory cleanup.
    ...
    // Always clear resident derived material, regardless of durable failures.
    this._clearInMemoryState();
    this._biometricState = 'unknown';
    this._lastBackup = null;
    this._lastRestore = null;

    // 4. Surface the first captured failure (if any).
  ...
  private _clearInMemoryState() {
    ...
    this._secretBytes = undefined;
    this._rootSeed = undefined;
    // `_rootHdKey` was removed entirely (see field
  ...
  /** Persist biometric routing state for the next cold launch. */
  ...
  /**
   * Detect whether this device has had a working biometric vault.
   *
   ...
   * prefs, so this check works uniformly across
   * both platforms.
   *
