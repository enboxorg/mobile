...
/**
 * The `EnboxUserAgent` constructs `SyncEngineLevel` with
 ...
 *
 * This only swallows the known idempotency path. The message patterns
 * are kept lower-cased because react-native-leveldb's underlying
 * LevelDB JNI wrapper embeds the path into the message and the case of
 * "Does not exist" varies across Android API levels.
 *
 * The "is not a function" pattern is deliberately excluded. It once
 * let test mocks that omit a `destroyDB` spy resolve cleanly, but the
 * same predicate runs in PRODUCTION — and
 ...
 *
 * This only swallows known idempotency failures. A real I/O failure, permission
 ...
 * even though stale identity / DWN bytes remained on disk. Anything
 * outside `isIdempotentDestroyError` is rethrown so the
 ...
 *
 * The wipe attempts every location before reporting failure:
 ...
 *
 * The wipe also destroys the root `dataPath` itself. That is where
 * `SyncEngineLevel` opens its replication-ledger /
...
export async function destroyAgentLevelDatabases(dataPath: string): Promise<void> {
  ...
  // Destroy the root path itself (sync engine DB).
