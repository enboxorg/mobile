 ...
 * If a later step throws, the catch path must zero the unlocked vault
 * before dropping store references so secret bytes do not linger until
 * GC.
 *
 * The store captures a local `vaultRef` after `initializeAgent()` returns
 * and calls `vaultRef.lock()` defensively before re-throwing. This suite
 * pins that contract:
 *
...
// Parity for `restoreFromMnemonic()`.
//
...
//
// The restore catch path mirrors the other init paths: capture `vaultRef`
// after `initializeAgent()` returns, and call `vaultRef.lock()` before
// dropping references.
describe('useAgentStore.restoreFromMnemonic() — defensive vault.lock() in catch path', () => {
