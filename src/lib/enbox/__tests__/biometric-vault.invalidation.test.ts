...
// ===========================================================================
// biometricState persist failures must log instead of silently swallowing
// ===========================================================================
//
...
describe('BiometricVault — persist-failure observability', () => {
  ...
  it('logs (and does not throw past the primary error) when persisting `invalidated` fails on KEY_INVALIDATED unlock', async () => {
    ...
    // The persist failure was logged; a silent `catch {}` here would
    // leave no observability.
  ...
  it('logs (and proceeds with provisioning) when post-initialize SecureStorage flag write fails', async () => {
    ...
    // natively at this point; orphan-secret recovery is the
    // next-launch fallback for missing flags.
    ...
    // The persist failure was logged with the documented post-initialize
    // context so on-call can correlate. Previously this was a silent
