/**
 * Regression tests for restoreFromMnemonic assigning agentDid.
 *
 * Background
 * ----------
 ...
 *
 * Restore assigns `agent.agentDid = await vault.getDid()` after
 * `agent.initialize({ recoveryPhrase })` succeeds, matching the
 * assignment upstream's `start()` performs without triggering a second
 * biometric prompt because the vault is already unlocked.
 *
 * These tests:
...
function makeFakeAgentAndVault(opts: {
  ...
  const agent: FakeAgent = {
    // Starts unset per upstream's contract; restore must assign it.
    agentDid: undefined,
