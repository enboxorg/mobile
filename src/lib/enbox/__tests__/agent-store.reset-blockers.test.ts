/**
 * Targeted regression tests for `useAgentStore.reset()`:
 *
 *   - reset() must wipe the persistent ENBOX_AGENT LevelDB data on disk
 *     so a post-reset relaunch doesn't resurrect identities / DWN
 *     records / sync cursors from the previous wallet. The wipe is
 *     delegated to `destroyAgentLevelDatabases` in `rn-level.ts`.
 *
 *   - reset()'s fallback path (no vault instance in the store, e.g.
 *     after `invalidated` recovery) must clear BOTH
 *     `enbox.vault.initialized` and `enbox.vault.biometric-state` from
 *     SecureStorage so a subsequent cold launch does not misroute away
 *     from clean onboarding.
 *
...
jest.mock(
  ...
  () => {
    ...
    return {
      __esModule: true,
      AuthManager: { create },
      // surface the canonical STORAGE_KEYS so
...
const { STORAGE_KEYS: AUTH_STORAGE_KEYS } = jest.requireMock('@enbox/auth') as {
...
// ===========================================================================
// reset() wipes persistent ENBOX_AGENT LevelDB data
// ===========================================================================
describe('useAgentStore.reset() — LevelDB wipe', () => {
  it('destroys every ENBOX_AGENT sub-database via react-native-leveldb.destroyDB', async () => {
    ...
    // (SyncEngineLevel opens its DB at the literal
    ...
    expect(destroyedNames).toEqual(expect.arrayContaining([
      ...
      // the root path itself (sync engine LevelDB).
      'ENBOX_AGENT',
    ]));
  });

  // A swallowed-error contract ("reset() resolves cleanly even when
  // destroyDB throws") hides genuine wipe failures from the caller and leaves stale
  // identities / DWN records on disk that the next
  // `initializeFirstLaunch()` resurrects via the LevelDB handle the
  // agent opens against `dataPath`. The contract is fail-loud:
  ...
  it('rethrows LevelDB.destroyDB failure AND persists a cleanup-pending sentinel', async () => {
    await useAgentStore.getState().initializeFirstLaunch();
  ...
  // cont.: regression test for the recovery path. After a
...
// ===========================================================================
// reset() fallback (no vault) clears SecureStorage keys
// ===========================================================================
describe('useAgentStore.reset() — no-vault fallback clears SecureStorage keys', () => {
  ...
  // Native wipe failures must remain visible while keeping retry state
  // armed for the next agent-init flow.
  it('rethrows native deleteSecret failure AND persists a vault-reset-pending sentinel', async () => {
    resetStoreState();
  ...
  it('next initializeFirstLaunch() retries the native wipe via the vault-reset sentinel', async () => {
    nativeSecure.getItem.mockImplementation(async (key: string) => {
  ...
  // the LevelDB cleanup helper now fails CLOSED on
  // SecureStorage read failures leave cleanup state unknown, so the
  // helper must throw instead of treating the sentinel as absent.
  it('runPendingLevelDbCleanup propagates SecureStorage.get failures so a stale LevelDB cannot leak through', async () => {
  ...
  // resumePendingBackup must gate on the same cleanup helpers as the
  // other init flows so backup-pending sessions cannot reopen stale
  // LevelDB or OS-gated vault state. The store-mocked `agent.start({})`
  ...
  it('resumePendingBackup retries the LevelDB wipe via the cleanup-pending sentinel BEFORE creating the agent', async () => {
    nativeSecure.getItem.mockImplementation(async (key: string) => {
      if (key === 'enbox:enbox.agent.leveldb-cleanup-pending') return 'true';
      return null;
    });
  ...
  // also assert resumePendingBackup runs the vault-reset
  // cleanup so a stale OS-gated secret cannot survive a backup-pending
  // resume. Same wiring approach as the LevelDB test above.
  it('resumePendingBackup retries the native vault wipe via the vault-reset sentinel BEFORE creating the agent', async () => {
    nativeSecure.getItem.mockImplementation(async (key: string) => {
      if (key === 'enbox:enbox.vault.reset-pending') return 'true';
      return null;
    });
  ...
  // parallel coverage for the AUTH_RESET sentinel —
  ...
  // a failed reset.
  it('resumePendingBackup retries the AUTH wipe via the auth-reset sentinel BEFORE creating the agent', async () => {
    nativeSecure.getItem.mockImplementation(async (key: string) => {
  ...
  it('initializeFirstLaunch retries the AUTH wipe via the auth-reset sentinel BEFORE creating the agent', async () => {
    nativeSecure.getItem.mockImplementation(async (key: string) => {
      if (key === 'enbox:enbox.auth.reset-pending') return 'true';
      return null;
    });
...
// ===========================================================================
// - runPendingVaultResetCleanup helper contract
// ===========================================================================
describe('runPendingVaultResetCleanup', () => {
  ...
  it('propagates SecureStorage.get failures so an unreadable sentinel cannot leak through (parity)', async () => {
    const { runPendingVaultResetCleanup } = require('@/lib/enbox/agent-store');
...
// ===========================================================================
// - useAgentStore.reset() fails CLOSED when sentinel write fails
// ===========================================================================
//
// Sentinel writes are the crash-recovery contract. If SecureStorage is
// not writable, reset must fail before touching the native vault, LevelDB,
// or session store.
describe('useAgentStore.reset() — fail-CLOSED on sentinel write failure', () => {
...
// ===========================================================================
// - LevelDB cleanup-pending sentinel is written BEFORE the wipe
// ===========================================================================
//
// The LevelDB sentinel must be written before the multi-subpath wipe
// starts and cleared only after the wipe succeeds.
describe('useAgentStore.reset() — LevelDB sentinel written BEFORE the wipe', () => {
  ...
  it('keeps LEVELDB_CLEANUP_PENDING_KEY set when destroyAgentLevelDatabases throws mid-wipe (crash resilience)', async () => {
    ...
    // Sentinel was written before the LevelDB wipe.
    const sentinelWrites = nativeSecure.setItem.mock.calls.filter(
      (c) => c[0] === 'enbox:enbox.agent.leveldb-cleanup-pending',
    );
    expect(sentinelWrites.length).toBeGreaterThanOrEqual(1);
    // Sentinel was not cleared because the wipe failed.
    const sentinelDeletes = nativeSecure.deleteItem.mock.calls.filter(
      (c) => c[0] === 'enbox:enbox.agent.leveldb-cleanup-pending',
    );
    expect(sentinelDeletes.length).toBe(0);
...
// ===========================================================================
// - session reset deferred until ALL critical wipes succeed
// ===========================================================================
//
// Session reset is deferred until all critical wipes succeed so Settings
// stays mounted and can surface the reset failure.
describe('useAgentStore.reset() — defers session-store reset on failure', () => {
  it('does NOT reset session-store when LevelDB wipe fails', async () => {
    ...
    // The session-store snapshot must be unchanged so the navigator
    // stays on Settings instead of moving to Loading.
  ...
  it('does NOT reset session-store when vault wipe fails', async () => {
    ...
    // A vault wipe failure must stop later persistent wipes. Otherwise
    // the next launch can see an intact vault plus erased LevelDB/auth
    // state and incorrectly classify retry sentinels as false alarms.
    expect(mockDestroyDB).not.toHaveBeenCalled();
    const deletedKeys = nativeSecure.deleteItem.mock.calls.map((c) => c[0]);
    for (const authKey of Object.values(AUTH_STORAGE_KEYS)) {
      expect(deletedKeys).not.toContain(`enbox:${authKey}`);
    }
  });
...
// ===========================================================================
// - wallet reset clears persisted AuthManager / Web5 connect material
// ===========================================================================
//
// Wallet reset must also clear AuthManager's `enbox:auth:*` keys so
// dApp delegate keys, active identity, registrations, and revocations do
// not carry into the next wallet. `AUTH_RESET_PENDING_KEY` guards against
// crash-during-iteration leaks, and `runPendingAuthResetCleanup()`
// retries the iteration on the next agent-init.
describe('useAgentStore.reset() — wipes persisted AuthManager material', () => {
  ...
  it('runPendingAuthResetCleanup propagates SecureStorage.get failures (parity)', async () => {
    const { runPendingAuthResetCleanup } = require('@/lib/enbox/agent-store');
    const stubError = Object.assign(new Error('SecureStorage temporarily unavailable'), {
      code: 'SECURE_STORAGE_LOCKED',
    });
...
// ===========================================================================
// - sentinel rollback covers values that landed before trackKey
// ===========================================================================
//
...
// `set()` rejects after the value write but before key tracking, reset
// must roll back every sentinel in `SENTINEL_KEYS`. `remove()` is
// idempotent on absent keys, so unconditional rollback is safe.
describe('useAgentStore.reset() — sentinel rollback covers partial-success writes', () => {
  it('removes the partially-persisted sentinel value when set() rejects after the value landed', async () => {
    ...
    nativeSecure.setItem.mockImplementation(async (key: string, _value: string) => {
      ...
      if (key === 'enbox:__keys__') {
        ...
        if (!(global as any).__sentinelRollbackKeyIndexThrew) {
          (global as any).__sentinelRollbackKeyIndexThrew = true;
          throw trackKeyError;
        }
      }
      onDiskKeys.add(key);
    ...
    try {
      ...
      // → reset() rolls back ALL sentinels.
      await expect(useAgentStore.getState().reset()).rejects.toThrow(
        /KEY_INDEX disk full/,
      );
      ...
      delete (global as any).__sentinelRollbackKeyIndexThrew;
    }
  });
  ...
  it('rolls back every SENTINEL_KEY (vault + leveldb + auth + session) on a sentinel-write failure', async () => {
    ...
    expect(deletedKeys).toEqual(
      expect.arrayContaining([
        ...
        // SESSION_RESET_PENDING_KEY is the fourth
...
// ===========================================================================
// - session-reset sentinel guards the ghost-state misroute
// ===========================================================================
//
// SESSION_RESET_PENDING_KEY stays set until session.reset() resolves.
// `session.hydrate()` checks the sentinel before reading SESSION_KEY
// so ghost state cannot route to BiometricUnlock against a wiped vault.
// Welcome flow.
describe('useAgentStore.reset() — session-reset sentinel', () => {
  ...
  it('keeps SESSION_RESET_PENDING_KEY set when SESSION_KEY deletion fails (ghost-state guard)', async () => {
    ...
    // leaving only the session ghost-state guard armed.
...
// ===========================================================================
// - rollback failure surfacing + vault-intact defensive guard
// ===========================================================================
//
// made the rollback iterate every SENTINEL_KEY (not just
...
//       still live on disk after a failed reset attempt. The current implementation
...
//
//       The current implementation adds an `isVaultStateIntact()` defensive guard at
...
describe('useAgentStore.reset() — rollback failure surfacing', () => {
  it('aggregates rollback failures into the thrown error so callers can detect a stale on-disk sentinel', async () => {
    ...
    nativeSecure.setItem.mockImplementation(async (key: string, _value: string) => {
      if (key === 'enbox:__keys__') {
        ...
        if (!(global as any).__rollbackAggregationKeyIndexThrew) {
          (global as any).__rollbackAggregationKeyIndexThrew = true;
    ...
    // also fails. The aggregated error must surface this so the caller
    // knows the sentinel is still live on disk.
    nativeSecure.deleteItem.mockImplementation(async (key: string) => {
  ...
  it('throws the original sentinel-write error UNWRAPPED when rollback succeeds (no false-positive aggregation)', async () => {
    ...
    nativeSecure.setItem.mockImplementation(async (key: string) => {
      if (key === 'enbox:__keys__') {
        if (!(global as any).__rollbackSuccessKeyIndexThrew) {
          (global as any).__rollbackSuccessKeyIndexThrew = true;
          throw new Error('KEY_INDEX transient IO error');
        }
      }
    });
    // Rollback delete succeeds for every key, so the thrown error stays unwrapped.
    ...
    try {
      ...
      delete (global as any).__rollbackSuccessKeyIndexThrew;
    }
  });
});

describe('runPendingResetCleanups — vault-intact defensive guard', () => {
  ...
  it('SKIPS destructive cleanups when sentinels are set + vault is intact (false-alarm path)', async () => {
    ...
    // sentinels are also set — exactly the partial-
