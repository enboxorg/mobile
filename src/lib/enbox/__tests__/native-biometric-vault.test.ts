...
// Keeping this as a frozen sorted tuple makes the "ten codes, no more, no less"
...
// The native API surface must refuse to overwrite an existing secret.
// Callers that intend to replace a wallet root must delete it explicitly
// before provisioning a new alias. The contract is enforced at three layers:
//
...
// Regression coverage for native failure safety:
// - Any rejection from `generateAndStoreSecret(alias, ...)` must preserve a
//   pre-existing alias byte-for-byte.
// - A `KEY_INVALIDATED` rejection from `getSecret(alias, ...)` must be
//   followed by `hasSecret(alias) === false`, so the UI can route to recovery.
describe('NativeBiometricVault — native failure safety regressions', () => {
  const prompt = {
    promptTitle: 'Unlock',
    promptMessage: 'Authenticate',
    promptCancel: 'Cancel',
  ...
  it('preserves the pre-existing alias when generateAndStoreSecret rejects after a transient probe failure', async () => {
    const alias = 'enbox.wallet.root';
    const originalHex =
    ...
    // `generateAndStoreSecret(alias, ...)` call. Native must reject with
    // `VAULT_ERROR` instead of deleting the existing alias.
  ...
  it('clears the alias before rejecting getSecret with KEY_INVALIDATED', async () => {
    const alias = 'enbox.wallet.root';
    await expect(
      NativeBiometricVault.generateAndStoreSecret(alias, {
        requireBiometrics: true,
    ...
    // The alias must be gone after the KEY_INVALIDATED rejection so the
    // user is not trapped in a repeated invalidation loop.
    await expect(NativeBiometricVault.hasSecret(alias)).resolves.toBe(false);
    await expect(
      NativeBiometricVault.getSecret(alias, prompt),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
  ...
  it('allows re-provisioning immediately after a KEY_INVALIDATED rejection', async () => {
    const alias = 'enbox.wallet.root';
    const originalHex =
...
describe('NativeBiometricVault — strict lower-case secretHex contract', () => {
  it('Jest mock rejects uppercase secretHex with VAULT_ERROR (parity with Android LOWER_HEX_64_REGEX + iOS lowercase-only parser)', async () => {
    // The TurboModule spec, native modules, and Jest mock all require
    // exactly 64 lower-case hex characters. This test pins the JS-side
    // mirror of that regex so any future regression on
...
describe('NativeBiometricVault — secretHex empty-string parity', () => {
  // The Jest mock must use `providedHex !== null`, not a truthiness check,
  // because `secretHex: ""` is supplied-but-invalid input.
  // A truthiness check is JS-falsy
  ...
  // `generateAndStoreSecret(..., { secretHex })`. Any string the caller
  // supplies — including `""` — must be funnelled through the same regex
  // check the native parsers use.

  it('rejects secretHex: "" with VAULT_ERROR', async () => {
    const alias = 'enbox.wallet.root';
    ...
    // CSPRNG-derived bytes that the JS layer never asked for).
    await expect(NativeBiometricVault.hasSecret(alias)).resolves.toBe(false);
  });

  it('omitting secretHex still falls through to deterministic CSPRNG', async () => {
    const alias = 'enbox.wallet.root';
  ...
  it('treats secretHex: undefined like the omitted-key path', async () => {
    const alias = 'enbox.wallet.root';
...
describe('NativeBiometricVault — getSecret success path', () => {
  ...
  it('getSecret resolves a valid lowercase hex string once the alias has been provisioned', async () => {
    // Populate the coherent store first; the default mock rejects with
    // NOT_FOUND when the alias is absent.
...
describe('NativeBiometricVault — default mock is internally coherent', () => {
  const prompt = {
    promptTitle: 'Unlock',
    promptMessage: 'msg',
    promptCancel: 'cancel',
...
describe('NativeBiometricVault — requireBiometrics flag semantics', () => {
  ...
  it('DEFAULT mock (no mockRejectedValueOnce) ALSO rejects on requireBiometrics=false — parity with native, no test override needed', async () => {
    // The default mock must mirror the native behaviour by default — no
    // one-off mock override should be required for this rejection.
    //
  ...
  it('DEFAULT mock accepts requireBiometrics=true (no false-positive rejection that would block every other test)', async () => {
    // The guard must not over-reject the canonical
    // ``requireBiometrics: true`` payload that production callers use.
    await expect(
      NativeBiometricVault.generateAndStoreSecret('enbox.wallet.root', {
        requireBiometrics: true,
        invalidateOnEnrollmentChange: true,
  ...
  it('DEFAULT mock accepts requireBiometrics OMITTED (treated as default-true on native; mock matches)', async () => {
...
describe('NativeBiometricVault — cross-platform error-code union invariance', () => {
  ...
  // ten codes are produced: BiometricPrompt.ERROR_LOCKOUT_PERMANENT maps
  ...
  it('the union of iOS ∪ Android codes equals the full ten-code canonical set', () => {
