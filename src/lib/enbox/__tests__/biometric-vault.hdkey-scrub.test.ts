/**
 * HD-key buffer scrubbing.
 *
 * BiometricVault must not store the root HDKey on `this._rootHdKey`.
 * No consumer reads that field, and the HDKey instance retains a
 ...
 *
 * The implementation zeroes every HDKey's `chainCode` + `privateKey`
 * at the derivation sites:
...
describe('HD-key buffer scrubbing', () => {
  it('BiometricVault.prototype does NOT declare a _rootHdKey slot (regression guard)', () => {
    // A `private _rootHdKey: any | undefined;` class-property
    ...
    // removing the field means a fresh instance has NO
    // `_rootHdKey` own-key.
  ...
  it('captures the rootHdKey via a custom didFactory and asserts chainCode+privateKey are zero AFTER initialize()', async () => {
    ...
    // initializing, by which point the `_doInitialize` finally-block
    // should have zeroed both buffers in place.
