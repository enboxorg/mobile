...
/**
 * SecureStorage sentinel that signals "`useAgentStore.reset()` may
 * have reached durable wipe steps while SESSION_KEY was still on disk".
 *
 * `hydrate()` checks this before trusting SESSION_KEY. If the vault is
 * wiped or indeterminate, SESSION_KEY is treated as absent so the
 * navigator routes to Welcome instead of BiometricUnlock against a
 * wiped vault. If the vault is proven intact, the sentinel is treated
 * as stale and SESSION_KEY remains authoritative.
 *
 ...
 * directly so orphan-secret recovery can detect that the vault was
 * provisioned without depending on the separate Welcome
 * `hasCompletedOnboarding` write, which is a
...
export interface SessionState {
   ...
   *
   * This helper is the single source of truth for committing a
   * successful restore.
   */
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  hydrate: async () => {
    try {
      // Read the vault's own `INITIALIZED_STORAGE_KEY` sentinel. The
      // orphan-secret recovery below uses it as a durable
      // "user had a working vault" signal independent of the
      // fire-and-forget Welcome `hasCompletedOnboarding` write.
      //
      // Also read SESSION_RESET_PENDING_RAW_KEY before trusting
      // SESSION_KEY. The sentinel protects cold-launch routing after a
      // partial reset, while still allowing a proven-intact vault to
      // keep its session snapshot.
      ...
      let sessionResetGuardActive = false;
      let knownHasSecret: boolean | null = null;
      // If the session-reset sentinel is set and the vault is not
      // provably intact, ignore any persisted SESSION_KEY. If the vault
      // is intact, the sentinel is stale and is cleared without
      // touching SESSION_KEY.
      if (sessionResetPending === 'true') {
        let vaultStillIntact = false;
        if (rawVaultInitialized === 'true') {
          const probedHasSecret = await NativeBiometricVault.hasSecret(
            WALLET_ROOT_KEY_ALIAS,
          ).catch(() => null);
          if (typeof probedHasSecret === 'boolean') {
            knownHasSecret = probedHasSecret;
            vaultStillIntact = probedHasSecret;
          }
        }

        if (vaultStillIntact) {
          console.warn(
            '[session] hydrate: SESSION_RESET_PENDING sentinel looked stale while vault is intact; preserving SESSION_KEY',
          );
          try {
            await deleteSecureItem(SESSION_RESET_PENDING_RAW_KEY);
          } catch (err) {
            console.warn(
              '[session] hydrate: failed to clear stale SESSION_RESET_PENDING sentinel (next cold launch will re-evaluate the vault-intact guard):',
              err,
            );
          }
        } else {
          sessionResetGuardActive = true;
          console.warn(
      ...
      if (!sessionResetGuardActive && rawSession) {
        try {
      ...
      const hasSecret =
        knownHasSecret ??
        (await NativeBiometricVault.hasSecret(WALLET_ROOT_KEY_ALIAS).catch(
          () => false,
        ));
      ...
      //
      // The original orphan condition required
      ...
      // When the session-reset sentinel still represents a real or
      // indeterminate reset, refuse orphan-secret promotion. Otherwise
      // an attempted reset could route to RecoveryPhrase for the old
      // wallet. If the vault was proven intact above, the guard is
      // inactive and the normal orphan recovery path is preserved.
      const isOrphanedSecret =
        !sessionResetGuardActive &&
      ...
      // `hasCompletedOnboarding` back on. A completed `_doInitialize()`
      // means this is not a fresh install, even if the Welcome persist
      // did not land before process death.
  ...
  reset: async () => {
    // Delete SESSION_KEY first and only clear reset sentinels after it
    // succeeds. If SESSION_KEY remains on disk, hydrate() still needs
    // SESSION_RESET_PENDING as the ghost-session guard.
    await deleteSecureItem(SESSION_KEY);
    await Promise.all([
      ...
      // `hydrate()` uses this as an orphan-recovery signal, so reset must
      // clear it along with the rest of the session-visible state.
      deleteSecureItem(VAULT_INITIALIZED_RAW_KEY).catch(() => undefined),
      // clear the session-reset sentinel too so a
