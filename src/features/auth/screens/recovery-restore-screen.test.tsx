...
jest.mock('@/lib/enbox/agent-store', () => {
  ...
  // The recovery-restore screen must call
...
describe('RecoveryRestoreScreen', () => {
  ...
  // ------------------------------------------------------------------
  // Orphaned-vault zeroization on persistence failure.
  //
  ...
  // The catch block must do more than display a retry alert: it also
  // tears down the store so unlocked vault material is zeroed before
  // references are dropped.
  // ------------------------------------------------------------------
  it('tears down the agent-store when hydrateRestored rejects', async () => {
    (mockRestoreFromMnemonic as jest.Mock).mockResolvedValue(undefined);
  ...
  // Teardown failures inside the catch block must not break the
  // user-visible retry path. A teardown that throws is
  // logged and swallowed; the inline retry alert is still rendered
  // and the input remains populated so the user can resubmit.
  it('renders retry alert even if teardown() also throws', async () => {
    (mockRestoreFromMnemonic as jest.Mock).mockResolvedValue(undefined);
