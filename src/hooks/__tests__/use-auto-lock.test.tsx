 ...
 *   - The skip-when-locked guard also requires the agent-store to be
 *     empty (no agent / vault / recoveryPhrase). A
...
describe('useAutoLock', () => {
  ...
  it('TEARS DOWN even when isLocked=true if the agent is still resident after unlock', () => {
  ...
  it('TEARS DOWN even when isLocked=true if the vault is still resident during provisioning', () => {
    mockIsLocked = true;
    mockAgent = null;
  ...
  it('TEARS DOWN on background during first-launch RecoveryPhrase backup', () => {
    // Session is still locked because `unlockSession()` doesn't fire until the user
