 ...
 *     vault / recovery-phrase material to scrub, regardless of the
   *     session-store `isLocked` flag. See the `agent-resident`
   *     rationale below.
 ...
 *
   * Why we do not skip solely on `isLocked`:
 *
 *   A guard on `isLocked` alone short-circuited the entire handler
 *   whenever `useSessionStore.getState().isLocked` was already `true`. That
...
export function useAutoLock(): void {
  ...
  useEffect(() => {
    function handleAppStateChange(next: AppStateStatus): void {
      ...
      // Gate on whether the agent-store still holds any unlocked
      // material. We check `agent`, `vault`, and
