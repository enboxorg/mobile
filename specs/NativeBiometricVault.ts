...
export interface Spec extends TurboModule {
   ...
   * `generateAndStoreSecret` / `getSecret` / `deleteSecret` calls on the
   * same alias. A second concurrent call for an alias whose provisioning,
   * unlock, or deletion is still in flight MUST fail-fast with
   * `VAULT_ERROR_OPERATION_IN_PROGRESS`. Cross-alias calls remain parallel.
   *
   * iOS and Android both use per-alias in-flight membership because the
   * underlying work crosses asynchronous biometric callbacks. The JS layer
