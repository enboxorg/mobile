...
- (void)generateAndStoreSecret:(NSString *)keyAlias
  ...
  // Round-9 F3: read provisioning prompt copy. Used to drive the
  ...
  dispatch_async(_keychainQueue, ^{
    ...
    // Round-9 F3: explicit biometric confirmation BEFORE SecItemAdd.
    //
    ...
    // NOT prompt. Pre-Round-9 iOS provisioning therefore landed
    // bytes in the keychain with no user-presence check, breaking
    ...
    // never authenticated.
    //
    // The fix is an explicit `LAContext.evaluatePolicy` with
    ...
    [provisionContext evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics
      ...
      if (strongSelf == nil) {
        ...
        reject(kErrVault, @"Module was deallocated during provisioning", nil);
        return;
      }
      dispatch_async(strongSelf->_keychainQueue, ^{
        if (!evalSuccess) {
          [secretData resetBytesInRange:NSMakeRange(0, secretData.length)];
          if (capturedSacRef != NULL) CFRelease(capturedSacRef);
          NSString *code = kErrAuthFailed;
          if (evalError != nil) {
            if (evalError.code == LAErrorUserCancel ||
                evalError.code == LAErrorAppCancel ||
                evalError.code == LAErrorSystemCancel) {
              code = kErrUserCanceled;
            } else if (evalError.code == LAErrorBiometryLockout) {
              code = kErrBiometryLockout;
            } else if (evalError.code == LAErrorBiometryNotAvailable ||
                       evalError.code == LAErrorBiometryNotEnrolled) {
              code = kErrBiometryUnavailable;
            } else if (evalError.code == LAErrorAuthenticationFailed) {
              code = kErrAuthFailed;
            }
          }
          reject(code,
        ...
        // SecItemAdd path below. Implemented as a helper so the
        // pre-Round-9 control flow stays readable.
        [strongSelf finishGenerateAndStoreSecret:keyAlias
...
// Round-9 F3 helper: post-LAContext provisioning path. Runs the
