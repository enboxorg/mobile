...
static NSString *const kErrOperationInProgress = @"VAULT_ERROR_OPERATION_IN_PROGRESS";
// `generateAndStoreSecret` is intentionally not an upsert. Callers must
// delete explicitly before provisioning over an existing alias.
...
@implementation RCTNativeBiometricVault {
  ...
  NSMutableSet<NSString *> *_activeAliases;
}
...
- (instancetype)init {
  if ((self = [super init])) {
    ...
    _activeAliases = [NSMutableSet set];
  }
  return self;
}
...
// Must be called on `_keychainQueue`.
- (BOOL)beginAliasOperation:(NSString *)keyAlias
                  operation:(NSString *)operation
                   rejecter:(RCTPromiseRejectBlock)reject {
  if ([_activeAliases containsObject:keyAlias]) {
    NSString *message = [NSString stringWithFormat:
        @"A generateAndStoreSecret/getSecret/deleteSecret is already in progress for alias during %@",
        operation];
    reject(kErrOperationInProgress, message, nil);
    return NO;
  }
  [_activeAliases addObject:keyAlias];
  return YES;
}

// Must be called on `_keychainQueue`.
- (void)endAliasOperation:(NSString *)keyAlias {
  [_activeAliases removeObject:keyAlias];
}
...
- (void)generateAndStoreSecret:(NSString *)keyAlias
  ...
  // Read provisioning prompt copy. Used to drive the
  // explicit `LAContext.evaluatePolicy(...)` confirmation that runs
  // BEFORE `SecItemAdd` below. iOS's `SecItemAdd` with
  // `BiometryCurrentSet` does NOT prompt by itself; without an
  // explicit `LAContext` evaluation, provisioning would land bytes
  // in the keychain with no user-presence check whatsoever — a
  // direct contradiction of the BiometricSetup screen's
  // "Confirm biometrics to finish setup" copy and a trivially-
  // bypassable hole that any non-UI caller (a deep link, a
  // hijacked URL scheme, an attached debugger driving the
  // TurboModule directly) could exploit to seal a wallet on a
  // device whose owner never authenticated.
  NSString *provisionPromptMessage = [options[@"promptMessage"] isKindOfClass:[NSString class]]
      ? options[@"promptMessage"]
      : @"Confirm biometrics to finish setup";
  NSString *provisionPromptCancel = [options[@"promptCancel"] isKindOfClass:[NSString class]]
      ? options[@"promptCancel"]
      : nil;

  dispatch_async(_keychainQueue, ^{
    if (![self beginAliasOperation:keyAlias
                         operation:@"generateAndStoreSecret"
                          rejecter:reject]) {
      return;
    }
    ...
    // Explicit biometric confirmation BEFORE SecItemAdd.
    //
    // Background: Android's `BiometricPrompt` is a hard prerequisite
    // for `Cipher.doFinal(...)` on a biometric-bound Keystore key, so
    // Android provisioning naturally requires a biometric
    // authentication. iOS's `SecItemAdd` with a `BiometryCurrentSet`
    // ACL, however, gates only FUTURE reads — the add itself does
    // NOT prompt. Without an explicit policy evaluation, iOS provisioning
    // can land bytes in the keychain with no user-presence check, breaking
    // platform parity with Android's "Confirm biometrics" UX
    // (BiometricSetup copy / VAL-VAULT-033) and creating an
    // attack surface where any non-UI caller (a deep link, a
    // hijacked URL scheme, an attached debugger, a programmatic
    // test harness) could seal a wallet on a device whose owner
    // never authenticated.
    //
    // We use an explicit `LAContext.evaluatePolicy` with
    // `LAPolicyDeviceOwnerAuthenticationWithBiometrics`. The
    // localized reason is the caller-provided prompt message
    // (defaults to "Confirm biometrics to finish setup" — same
    // string the BiometricSetup screen advertises). On user-cancel
    // / authentication-failure the secret bytes are zeroed and
    // the access-control ref is released BEFORE rejecting, so
    // failure leaves no orphan state and no resident secret. On
    // success we re-enter `_keychainQueue` so the existence check
    // and `SecItemAdd` keep their serial-queue invariant.
    LAContext *provisionContext = [[LAContext alloc] init];
    if (provisionPromptCancel.length > 0) {
      provisionContext.localizedCancelTitle = provisionPromptCancel;
    }
    NSError *canEvalError = nil;
    BOOL canEvaluate = [provisionContext canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics
                                                     error:&canEvalError];
    if (!canEvaluate) {
      [secretData resetBytesInRange:NSMakeRange(0, secretData.length)];
      CFRelease(sacRef);
      NSString *code = kErrBiometryUnavailable;
      if (canEvalError != nil) {
        if (canEvalError.code == LAErrorBiometryNotEnrolled) {
          code = kErrBiometryNotEnrolled;
        } else if (canEvalError.code == LAErrorBiometryLockout) {
          code = kErrBiometryLockout;
        }
      }
      [self endAliasOperation:keyAlias];
      reject(code,
             @"Biometric authentication unavailable for provisioning",
             nil);
      return;
    }

    // Capture references that the LAContext reply block needs to
    // retain. The reply runs on a private LAContext thread; we
    // re-enter `_keychainQueue` inside the reply for SecItem* work
    // so the serial-queue invariant the rest of this module relies
    // on is preserved end-to-end.
    SecAccessControlRef capturedSacRef = sacRef;
    [provisionContext evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics
                     localizedReason:provisionPromptMessage
                               reply:^(BOOL evalSuccess, NSError *evalError) {
      dispatch_async(self->_keychainQueue, ^{
        if (!evalSuccess) {
          [secretData resetBytesInRange:NSMakeRange(0, secretData.length)];
          if (capturedSacRef != NULL) CFRelease(capturedSacRef);
          NSString *code = kErrAuthFailed;
          if (evalError != nil) {
            code = [self codeForLAError:evalError.code];
          }
          [self endAliasOperation:keyAlias];
          reject(code,
                 @"Biometric authentication failed during provisioning",
                 nil);
          return;
        }
        // Authenticated — fall through to the existence check +
        // SecItemAdd path below. Implemented as a helper so this
        // method stays readable.
        [self finishGenerateAndStoreSecret:keyAlias
                                secretData:secretData
                                    sacRef:capturedSacRef
                                   resolve:resolve
                                    reject:reject];
      });
    }];
...
// Helper: post-LAContext provisioning path. Runs the
// existence check + `SecItemAdd` on `_keychainQueue`. Takes
// ownership of `sacRef` (releases it on the
// already-initialized rejection path; transfers it to
// `SecItemAdd` on the success path via `__bridge_transfer`).
//
// Caller invariants:
//   * Already on `_keychainQueue`.
//   * `secretData` is the 32-byte secret (will be zeroed before
//     this method returns on every exit path).
//   * `sacRef` is non-NULL.
//   * Biometric authentication has already succeeded.
- (void)finishGenerateAndStoreSecret:(NSString *)keyAlias
                          secretData:(NSMutableData *)secretData
                              sacRef:(SecAccessControlRef)sacRef
                             resolve:(RCTPromiseResolveBlock)resolve
                              reject:(RCTPromiseRejectBlock)reject {
  // Refuse to provision over an existing alias. Callers that intend to
  // overwrite must delete explicitly before creating a new secret.
  {
